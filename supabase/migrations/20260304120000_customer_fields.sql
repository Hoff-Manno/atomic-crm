-- Add NZ customer-specific fields to contacts table
alter table contacts add column if not exists preferred_contact_method text default 'phone';
alter table contacts add column if not exists date_of_birth date;

-- Recreate contacts_summary view to include new columns
...
-- Update merge_contacts function to handle new fields
...
BEGIN
  ...
  UPDATE contacts SET
    ...
    preferred_contact_method = COALESCE(winner_contact.preferred_contact_method, loser_contact.preferred_contact_method),
